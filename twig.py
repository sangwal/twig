    ...
    while True:
        ...
        for column in range(2, 10):
            ...
            if len(clash_days) > 0:
                ...
                sheet.cell(row=row, column=column).value = CLASH_MARK + \
...
def main():
    ...
    tw_parser = subparsers.add_parser("teacherwise",
                                      help="Generate teacherwise timetable")
    tw_parser.add_argument('-f', '--fullname',
                           action='store_true',
                           help='replace short names with full names')
    tw_parser.add_argument("infile",
                           type=str,
                           action="store",
                           help="File containing classwise timetable")
